;(function () {
  ...
  class SectionScroll {
    constructor(element, config) {
      ...
      if (
        ...
        Object.getPrototypeOf(this) !== SectionScroll.prototype
      ) {
      ...
      if (config) {
        sectionScroll.config.sectionClass =
          config.sectionClass || sectionScroll.config.sectionClass
        sectionScroll.config.containerClass =
          config.containerClass || sectionScroll.config.containerClass
        sectionScroll.config.navDotContainer =
          config.navDotContainer || sectionScroll.config.navDotContainer
        sectionScroll.config.changeOnSectionColor =
          config.changeOnSectionColor ||
          sectionScroll.config.changeOnSectionColor
      }
  ...
  function _init() {
    // Sections list
    sectionScroll.sections = sectionScroll.container.querySelectorAll(
      `.${sectionScroll.config.sectionClass}`
    )
    ...
    if (sectionScroll.config.navDotContainer) {
      navDotContainer = document.querySelector(
        sectionScroll.config.navDotContainer
      )
    ...
    if (!navDotContainer) {
      if (sectionScroll.config.navDotContainer) {
        console.log(
          `${sectionScroll.config.navDotContainer} navDotContainer not found`
        )
      }
    } else {
    ...
    if (sectionScroll.dotNavWrapper) {
      sectionScroll.dotNavItems =
        sectionScroll.dotNavWrapper.querySelectorAll(".nav-item")
    }
    sectionScroll.sectionAnchorItems = []
    sectionScroll.sectionAnchorItems = document.querySelectorAll(
      "[data-section-anchor]"
    )
    // First active section will be the first section
    sectionScroll.activeSection = document.querySelector(
      ".page-main > .section"
    )

    // Section that will change on content-color
    sectionScroll.changeColorItems = document.querySelectorAll(
      sectionScroll.config.changeOnSectionColor
    )
  ...
  function _scrollHandler() {
    ...
    // Section anchor link
    sectionScroll.sectionAnchorItems.forEach(function (
      sectionAnchorItem,
      index
    ) {
      if (
        sectionScroll.activeSection.getAttribute("id") ===
        sectionAnchorItem.getAttribute("data-section-anchor")
      ) {
  ...
  function _resolveContainer(container) {
    if (container) {
      if (typeof container === "string") {
        ...
        console.log(
          'sectionScroll: invalid container "' + container + '" provided.'
        )
  ...
  function _isElementsSuperposed(el1, el2) {
    ...
    if (el1Rect.top <= el2Rect.bottom && el2Rect.top <= el1Rect.bottom) {
      return true
    }
    return false
  }
