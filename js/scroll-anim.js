  ...
  class ScrollAnim {
    constructor(element, config) {
      ...
      if (
        ...
        Object.getPrototypeOf(this) !== ScrollAnim.prototype
      ) {
      ...
      if (config) {
        ...
        scrollAnim.config.containerClass =
          config.containerClass || scrollAnim.config.containerClass
      }
  ...
  function _init() {
    ...
    if (!!window.IntersectionObserver) {
      ...
      scrollAnim.container
        .querySelectorAll("[data-anim-visible]")
        .forEach((element) => {
      ...
      // Fade in animation
      let fadeElts = scrollAnim.container.querySelectorAll(
      ...
      byLetterElts.forEach((byCharElt) => {
        ...
        if (byCharElt.getAttribute("data-anim-letter-delay")) {
          animLetterParam = parseFloat(
            byCharElt.getAttribute("data-anim-letter-delay")
          )
        }
        let animLineParam = 0.0
        if (byCharElt.getAttribute("data-anim-line-delay")) {
          animLineParam = parseFloat(
            byCharElt.getAttribute("data-anim-line-delay")
          )
        }
          ...
          if (line.tagName === "BR") {
            ...
            words.forEach((word) => {
              ...
              letters.forEach((letter) => {
                ...
                let animDelay =
                  animBegin +
                  letterIndex * animLetterParam +
                  lineIndex * animLineParam
      ...
      console.log(
        "Scroll anim disabled : Intersection observer not available on this browser"
      )
    }
  }
  ...
  function _resolveContainer(container) {
    if (container) {
      if (typeof container === "string") {
        ...
        console.log(
          'ScrollAnim: invalid container "' + container + '" provided.'
        )
